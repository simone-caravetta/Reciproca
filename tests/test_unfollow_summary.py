...
class UnfollowSummaryTest(unittest.TestCase):
    def setUp(self):
        ...
        R.config.UNFOLLOW_PROGRESS_FILE = os.path.join(tempfile.mkdtemp(), "unfollow_progress.json")
        R.state.uf_non_followers = [f"user{n}" for n in range(600)]
    ...
    @property
    def label(self):
        return R.gui.uf_data_label.settings.get('text', '')
    ...
    def test_without_a_loaded_list_it_asks_for_the_files(self):
        ...
        self.assertEqual(
            R.gui.uf_start_btn.state, 'disabled',
    ...
    def test_progress_for_users_no_longer_in_the_list_is_ignored(self):
        ...
        """
        R.state.uf_non_followers = ["still_here", "also_here"]
        self.save_progress(
